...
<template>
  <details class="identity-panel identity-panel--advanced">
    <summary>
      <span>Advanced</span>
      Node identity and browser trust
    </summary>
    <div class="identity-panel__body">
      <div>
      ...
      <div v-if="identity && !isConfirmingReset" class="identity-panel__trust-guidance">
        ...
        <ol>
          ...
          <li>
            Before installation, compare the complete SHA-256 fingerprint with the value shown
            above.
          </li>
        ...
        <p>
          Windows: import for the current user into Trusted Root Certification Authorities. macOS:
          add it to the login keychain and explicitly set trust. Linux and browser-specific stores:
          follow the platform administrator's certificate-authority procedure. These steps vary by
          release and are not automatically performed by LocalStream.
        </p>
      ...
      <p v-if="identity" class="identity-panel__trust-guidance">
        Installing this certificate grants your browser or operating system trust in this node.
        Export it only from this desktop, verify the full fingerprint above during installation, and
        remove it from the trust store if this node is reset or compromised. LocalStream never
        installs trust automatically.
      </p>
      <div v-if="identity && isConfirmingReset" class="identity-panel__confirmation" role="alert">
        <p>
          This removes the node identity and revokes every trusted device. All devices must pair
          again after LocalStream restarts.
        </p>
        <div class="identity-panel__actions">
          <button type="button" :disabled="isResetting" @click="$emit('cancelReset')">
            Cancel
          </button>
          <button
            type="button"
